...
- **Idempotency is mandatory:** processed cards get `data-kw-state` (`"active"` or `"spam"`), and `process()` skips tagged cards. Since the MutationObserver + runLoop reprocess everything, any new per-card work must be idempotent or it will duplicate (AI buttons, badges, fire emoji). Spam state is sticky for the session: removing a stop word in settings won't un-spam cards until page reload.
...
- **Infinite scroll fetches `?page=N` in a hidden iframe** and imports cards with `document.adoptNode`. The loader line in `updateState()` is dead code (`display: this.isEnabled ? "none" : "none"`) — loader visibility is set in `loadNextPage()`/`onIframeLoad()`. Don't "fix" it to keep the loader visible.
