...
- "Verification" is manual: run `node --check kwork-helper.user.js` first (the only fast syntax check — no build/test/lint exists), then install the file into Tampermonkey and test live on `https://kwork.ru/projects` (the `@match` only allows that URL), watching the browser console (F12).
...
- Release flow: bump `@version` in the header AND the version badge in README together (they must stay in sync), then push to `main` — users auto-update from the raw `main` URL, and Tampermonkey only applies an update when the version changes, so every pushed change must bump it.
- Commit style in this repo: short, lowercase, emoji-prefixed messages — not conventional commits. Code + README go in one `🦍💨 ...` commit; AGENTS.md changes get their own separate `📄 ...` commit.

## Code layout (one file, six classes)
...
- **Defensive processing:** per-card steps run inside the module-level `safe("label", fn)` wrapper (each distinct failure is logged to console once, not every 500 ms) so one broken selector can't kill the whole loop. New per-card work must be wrapped in `safe()` too. Critical selectors are registered in `CORE_SELECTORS`; the throttled `checkDomHealth()` (in `runLoop`, every ~10 s via `loopCount % 20`) warns once per selector when Kwork renames markup — keep new critical selectors in that map.
...
- **User-supplied values from localStorage** (stop words, API key, base URL, prompt) must be escaped with the module-level `escapeHtml()` before being injected into the settings modal HTML — never interpolate them raw.
